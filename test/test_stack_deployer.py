...
import json
from deploy_stack.deploy_stack import StackDeployer, LambdaCollection, VersionManifest, S3Uploadable, DirectoryScanner, CloudformationCollection, SSMParameters, ColorFormatter, StackParameters
...
class CommonClass():
    ...
    def load_init(self):
        ...
        testargs = ["PassArgs", "-i", "test-stack", "-e", "prod",
                    "-d", "test.unit.cld", "--parameters-dir", "test/parameters","--templates-dir", "test/cloudformation","--lambda-dir","test/src", "deploy"]
    ...
    def test_s3_uploadable(self):
        ...
        s3_uploadable = S3Uploadable('test/cloudformation/servicecatalog/v9/cf-provision-shared-services.cf.yaml',
                             s3Bucket,
                            'cloudformation/servicecatalog/v9/16fb196c9ed62b83167ff381565b5f0b76175332-cf-provision-shared-services.cf.yaml')
...
class TestDeploy(unittest.TestCase):
    ...
    def test_version_manifest(self, mock_session, mock_artifact):
        mock_artifact.return_value = {'release': {'release_version': 0, 'artifacts': ['Helloww']}}
    ...
    def test_check_md5(self, mock_session):
        ...
        check_md=obj_s3Upload.calculate_md5('test/cloudformation/servicecatalog/v9/cf-provision-shared-services.cf.yaml')
    ...
    def test_print_progress(self, mock_session):
        obj_common_class= CommonClass(mock_session)
        obj_s3Upload = obj_common_class.test_s3_uploadable()
        r = obj_s3Upload.print_progress(1000)
    ...
    def test_find_template(self, mock_session):
        ...
        obj = CloudformationCollection('test/cloudformation',obj_bucket,'test/cloudformation',{'ssm-parameters':{}});
        obj_cfn_template = obj.find_template('servicecatalog/v9/cf-provision-shared-services.cf.yaml')
        self.assertEqual(obj_cfn_template.name, 'servicecatalog/v9/cf-provision-shared-services.cf.yaml', 'Failed to pop the template from template folder')
    ...
    def test_find_template_file(self, mock_session):
        ...
        self.assertEqual(obj_cfn_template_file, 'test/cloudformation/support/kms-parameters-lambda.cf.yaml', 'Failed to pop the template from template folder')
    ...
    @patch('deploy_stack.deploy_stack.s')
    def test_format_role_pair(self, mock_session, mock_stack_lambda, mock_stack_output, mock_parse_parameters, mock_format_role_pair):
        class Options:
            def __init__(self):
        ...
        #mock_stack_parameters.read_parameters_yaml.return_value = {'stacks': [{'name': 'my-project-kms-decrypt-lambda','template': 'support/kms-parameters-lambda.cf.yaml','parameters': {'LambdaSourceS3Key': 'Test.zip'}}]}
